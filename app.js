...
products.forEach((product, index) => {
  product.addEventListener('touchmove', (e) => {
    ...
    if (touchX > products[currentIndex].getBoundingClientRect().left + products[currentIndex].offsetWidth / 2) {
      currentIndex++;
    } else if (touchX < products[currentIndex].getBoundingClientRect().left) {
      currentIndex--;
    }
  });
});
...
products.forEach((product, index) => {
  product.addEventListener('touchend', () => {
    console.log(`Swiped to product ${currentIndex + 1}`);
  });
});

document.addEventListener('scroll', () => {
  console.log(`Scrolled to product ${currentIndex + 1}`);
});
products[currentIndex].classList.add('selected');

if (currentIndex > 0) {
  products[currentIndex - 1].classList.remove('selected');
}
if (currentIndex > 0) {
  products[currentIndex - 1].classList.add('previous-selected');
}
if (currentIndex > 0) {
  products[currentIndex - 1].classList.remove('previous-selected');
